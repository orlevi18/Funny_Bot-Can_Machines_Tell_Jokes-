...
flags.DEFINE_string("data_path", "C:/Users/olevi/.spyder2-py3/models-master/tutorials/rnn/ptb/tmp/simple-examples/data/",
                    "Where the training/test data is stored.")
flags.DEFINE_string("save_path", "C:/Users/olevi/.spyder2-py3/models-master/tutorials/rnn/ptb/tmp/simple-examples/data/save_joke_wocka",
                    "Model output directory.")
flags.DEFINE_bool("use_fp16", False,
                  "Train using 16-bit floats instead of 32bit floats")
flags.DEFINE_bool("restore_model",False,
                  "restore_model")
...
def do_sample(session, model, word_2_id):
  ...
  for x in data:
    feed_dict = {}
    feed_dict[model._input.input_data] = [[x]]
  ...
  while True:
    feed_dict = {}
    feed_dict[model._input.input_data] = [[samples[-1]]]
    for layer_num, (c, h) in enumerate(model.initial_state):
      feed_dict[c] = state[layer_num].c
      feed_dict[h] = state[layer_num].h
    state, sample = session.run(fetches, feed_dict)
    word=sample[0][0]
    if word==word_2_id["EOJ"]:
        break
    
    if word!=word_2_id["UNK"]:
        samples.append(word)
...
def main(_):
  ...
  config = get_config()
  raw_data = reader.ptb_raw_data(config.vocab_size,FLAGS.data_path)
  train_data,valid_data,test_data,word_2_id = raw_data
  print(len(word_2_id))
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      if FLAGS.restore_model:
            print("restoring...")
            last_check=tf.train.latest_checkpoint(FLAGS.save_path)
            sv.saver.restore(session,last_check)
            for i in range(1,5):
              print("joke # "+str(i)+"\n")
              print(pretty_print(do_sample(session, mtest,word_2_id),False,id_2_word))        
      else:  
